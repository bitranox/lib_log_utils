...
function run_pytest() {
  ...
  if ! python3 -m pytest "${project_root_dir}" "$@" --cov=lib_log_utils; then
...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : $HOME/venv/bin/log_util --version"
  if ! "$HOME/venv/bin/log_util" --version; then
