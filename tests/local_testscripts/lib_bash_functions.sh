...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : /usr/local/bin/log_util --version"
  if ! "/usr/local/bin/log_util" --version; then
...
function test_commandline_interface_venv_old() {
  ...
  clr_green "issuing command : $HOME/venv/local/bin/log_util --version"
  if ! "$HOME/venv/local/bin/log_util" --version; then
    my_banner_warning "test commandline interface on virtual environment ERROR"
    beep
    sleep "${sleeptime_on_error}"
    return 1
